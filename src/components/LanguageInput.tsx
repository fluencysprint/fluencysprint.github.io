...
export default function LanguageInput({
  ...
  const baseClass = `w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition-all text-base bg-white dark:bg-slate-700 disabled:opacity-50 ${className}`;
        ...
        <div className="flex flex-wrap gap-1" data-testid="accent-toolbar">
          {SPANISH_CHARS.map(({ char, label }) => (
            <button
              ...
              disabled={disabled}
              className="px-2 py-1 text-sm rounded-lg bg-slate-100 dark:bg-slate-700 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 hover:text-indigo-700 dark:hover:text-indigo-300 text-slate-600 dark:text-slate-300 font-medium transition-colors disabled:opacity-40 select-none"
              title={char}
            >
              {label}
            </button>
