...
export default function Settings() {
  ...
  const select = <T extends string>(
    ...
    <select
      ...
      className="text-sm border border-slate-200 rounded-xl px-3 py-1.5 text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white"
    >
    ...
    <div className="space-y-6 pb-10">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Manage your profile, practice and data</p>
        </div>
        {savedFlash && (
      ...
      {/* Profile */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-5" data-testid="profile-section">
        <div className="text-xs text-slate-400 uppercase tracking-wide font-semibold pt-4 pb-2">Profile</div>
        {profile && pack && (
          <>
            {row('Active profile', `${pack.metadata.label}${profile.displayName ? ` · ${profile.displayName}` : ''} · target ${profile.targetLevel}`,
              <span className="text-xs text-slate-500">{profile.id.slice(0, 6)}…</span>
            )}
            {profiles.length > 1 && (
              <div className="py-3 border-b border-slate-100">
                <div className="text-xs text-slate-400 mb-2">Switch profile</div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-5">
        <div className="text-xs text-slate-400 uppercase tracking-wide font-semibold pt-4 pb-2">Practice</div>
        ...
        {row('Show per-question timer', 'Display seconds spent on each item',
          <button
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${settings.showTimers ? 'bg-indigo-600' : 'bg-slate-200'}`}
      ...
      {pack?.metadata.id === 'spanish' && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-5">
          <div className="text-xs text-slate-400 uppercase tracking-wide font-semibold pt-4 pb-2">Spanish input</div>
      ...
      {/* Data */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-5">
        <div className="text-xs text-slate-400 uppercase tracking-wide font-semibold pt-4 pb-2">Backup &amp; data</div>

        <div className="py-4 border-b border-slate-100">
          <div className="text-sm font-medium text-slate-800 mb-1">Export backup</div>
          <div className="text-xs text-slate-400 mb-3">All profiles and their data as one JSON file. Use this to move between devices.</div>
          <button
        ...
        <div className="py-4 border-b border-slate-100">
          <div className="text-sm font-medium text-slate-800 mb-1">Import backup</div>
          <div className="text-xs text-slate-400 mb-3">Replaces all current app data with the imported snapshot.</div>
          <label className="inline-block px-4 py-2 rounded-xl bg-slate-100 text-slate-700 text-sm font-semibold hover:bg-slate-200 cursor-pointer">
            Choose JSON file
            <input type="file" accept=".json" onChange={handleImport} className="hidden" />
          </label>
          {importStatus === 'success' && (
            <div className="mt-2 text-xs text-emerald-600">✓ Imported — refresh to see all changes.</div>
          )}
          {importStatus === 'error' && (
        ...
        <div className="py-4 border-b border-slate-100">
          <div className="text-sm font-medium text-amber-700 mb-1">Reset current profile</div>
          <div className="text-xs text-slate-400 mb-3">Wipes progress, mistakes, sessions and drafts for {profile?.displayName ?? 'this profile'} only.</div>
          <button
            onClick={handleResetProfile}
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${resetProfileConfirm ? 'bg-amber-600 text-white' : 'bg-amber-50 text-amber-700 hover:bg-amber-100'}`}
          >
        ...
        <div className="py-4 border-b border-slate-100">
          <div className="text-sm font-medium text-red-700 mb-1">Delete current profile</div>
          <div className="text-xs text-slate-400 mb-3">Removes the entire profile. If it is the only one, you will land back on onboarding.</div>
          <button
            onClick={handleDeleteProfile}
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${deleteProfileConfirm ? 'bg-red-600 text-white' : 'bg-red-50 text-red-600 hover:bg-red-100'}`}
          >
        ...
        <div className="py-4">
          <div className="text-sm font-medium text-red-700 mb-1">Reset all app data</div>
          <div className="text-xs text-slate-400 mb-3">Wipes every profile and every byte of local data. Cannot be undone.</div>
          <button
            onClick={handleResetAll}
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${resetAllConfirm ? 'bg-red-600 text-white' : 'bg-red-50 text-red-600 hover:bg-red-100'}`}
          >
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
        <div className="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-3">About</div>
