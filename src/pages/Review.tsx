...
export default function Review() {
  ...
  if (phase === 'reviewing') {
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="bg-amber-50 dark:bg-amber-900/40 border border-amber-200 dark:border-amber-700 rounded-xl px-4 py-2.5 text-xs text-amber-700 dark:text-amber-300">
  ...
  if (phase === 'done') {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Review complete</h1>
  ...
  return (
    <div className="space-y-6 pb-10">
      <div>
        <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Mistake review</h1>
        <p className="text-slate-400 text-sm mt-1">Spaced repetition over your tracked mistakes</p>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <div className="bg-red-50 dark:bg-red-900/40 rounded-2xl p-3 text-center border border-red-100 dark:border-red-800">
          <div className="text-2xl font-bold text-red-600">{overdueMistakes.length}</div>
          <div className="text-xs text-red-400">Overdue</div>
        </div>
        <div className="bg-amber-50 dark:bg-amber-900/40 rounded-2xl p-3 text-center border border-amber-100 dark:border-amber-800">
          <div className="text-2xl font-bold text-amber-600">{dueMistakes.length}</div>
          <div className="text-xs text-amber-400">Due today</div>
        </div>
        <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl p-3 text-center border border-slate-100 dark:border-slate-700">
      ...
      <div className="space-y-2">
        ...
        {dueMistakes.length === 0 && (
          <div className="bg-emerald-50 dark:bg-emerald-900/40 border border-emerald-200 dark:border-emerald-700 rounded-xl p-4 text-center text-sm text-emerald-700 dark:text-emerald-300">
        ...
        <button
          ...
          className="w-full py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-40"
        >
            ...
            {topCats.map(([cat, count]) => {
              ...
              return (
                <button
                  ...
                  className="text-left px-3 py-2.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20"
                >
                  <div className="text-sm font-medium text-slate-700 dark:text-slate-200">{MISTAKE_LABELS[cat]}</div>
      ...
      {nonMasteredMistakes.length === 0 && (
        <div className="text-center py-12">
          <div className="text-4xl mb-3">🎉</div>
          <div className="text-lg font-bold text-slate-700 dark:text-slate-200">No active mistakes!</div>
